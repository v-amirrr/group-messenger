...
const Login = () => {
    ...
    return (
        <>
            <LoginPage initial='hidden' animate='visible' exit='exit' variants={authVariants} loginloading={loginDataFromUserStore.loading ? 1 : 0} googleloading={googleLoginDataFromUserStore.loading ? 1 : 0} showpassword={showPassword ? 1 : 0}>
                <Notification />
                <div className='login-container'>
                    <motion.h2 variants={authItemVariants}>Login with your account</motion.h2>
                    <ul className='login-fields'>
                        <motion.div className='login-field' variants={authItemVariants}>
                            <input type='email' placeholder='Email' value={email} onChange={(e) => setEmail(e.target.value)} autoFocus disabled={loginDataFromUserStore.loading ? 1 : 0}/>
                        </motion.div>
                        <motion.div className='login-field' variants={authItemVariants}>
                            <input type={showPassword ? 'text' : 'password'} placeholder='Password' value={password} onChange={(e) => setPassword(e.target.value)} onPaste={(e) => e.preventDefault()} disabled={loginDataFromUserStore.loading ? 1 : 0}/>
                            <button className='login-password-button' onClick={() => setShowPassword(!showPassword)} disabled={loginDataFromUserStore.loading ? 1 : 0}>
                                <i><FaRegEye /></i>
                                <hr />
                            </button>
                        </motion.div>
                    </ul>
                    <motion.div className='login-buttons' variants={authItemVariants}>
                    ...
                    <motion.div className='login-google' variants={authItemVariants}>
                        <button className='google-submit' onClick={googleLoginHandler} disabled={loginDataFromUserStore.loading}>
    ...
    .login-container {
        ...
        .login-buttons {
            ...
            .back-button, .submit-button {
                ...
                @media (hover: hover) and (pointer: fine) and (min-width: 745px) {
                    &:hover {
                        background-color: ${props => props.loginloading ? "" : "var(--normal-bg-hover)"};
                    }
                }
            }
        ...
        .login-google {
            ...
            .google-submit {
                ...
                @media (hover: hover) and (pointer: fine) and (min-width: 745px) {
                    &:hover {
                        background-color: ${props => props.googleloading ? "" : "var(--normal-bg-hover)"};
                    }
                }
            }
            ...
            .google-loader {
                ...
                opacity: ${props => props.googleloading ? "1" : "0"};
